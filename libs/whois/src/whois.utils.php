...
class utils extends Whois {
	...
	function showHTML($result, $link_myself=true, $params='query=$0&amp;output=nice') {
		...
		$out = strip_tags($out);
		$out = preg_replace ($email_regex, '<a href="mailto:$0">0 تومانءءء</a>', $out);
		...
		if ($link_myself)
			...
			$out = preg_replace ($ip_regex, '<a href="'.$link.'">0 تومانءءء</a>', $out);
		...
		$out = preg_replace ("/(?m)^(%.*)/", '<i>0 تومانءءء</i>', $out);
